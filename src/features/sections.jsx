...
export function MapSection() {
  ...
  const panLimitX = 220 * zoom;
  const panLimitY = 140 * zoom;

  const onMouseDown = (e) => setDrag({ startX: e.clientX, startY: e.clientY, base: { ...offset } });
  const onMouseMove = (e) => {
    if (!drag) return;
    const dx = (e.clientX - drag.startX);
    const dy = (e.clientY - drag.startY);
    setOffset({
      x: clamp(drag.base.x + dx, -panLimitX, panLimitX),
      y: clamp(drag.base.y + dy, -panLimitY, panLimitY)
    });
  };
  ...
  const onWheel = (e) => {
    ...
    setZoom((z) => clamp(Math.round((z + dir * 0.15) * 100) / 100, 1, 2.6));
  };

  const worldPath = "M78 250c50-70 105-110 165-125 55-14 118-6 175 20 53 24 103 18 150-4 45-21 95-31 148-26 58 5 116 29 170 64 26 17 50 38 72 62-20 62-56 116-104 156-58 48-133 78-214 78-55 0-105-13-153-33-44-18-87-22-132-10-46 12-93 8-140-6-55-17-107-46-160-74-33-17-65-33-97-42 18-23 38-44 60-60Z";
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'monospace' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        <button onClick={() => setZoom((z) => clamp(Math.round((z + 0.2) * 10) / 10, 1, 2.6))} style={{ padding: '8px 10px', background: c.bg.card, border: '1px solid ' + c.border.default, cursor: 'pointer', color: c.text.primary, fontSize: 10 }}>ZOOM +</button>
        <button onClick={() => setZoom((z) => clamp(Math.round((z - 0.2) * 10) / 10, 1, 2.6))} style={{ padding: '8px 10px', background: c.bg.card, border: '1px solid ' + c.border.default, cursor: 'pointer', color: c.text.primary, fontSize: 10 }}>ZOOM -</button>
        ...
        <div style={{ marginLeft: 'auto', fontSize: 10, color: c.text.muted, display: 'flex', alignItems: 'center' }}>
          Scroll to zoom • Drag to pan • {locations.length} pins
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        ...
        >
          <svg width="100%" height="420" viewBox="0 0 1000 500" style={{ background: '#0b0b0b', border: '1px solid ' + c.border.default }}>
            <path d={worldPath} fill="rgba(255,255,255,0.03)" stroke="rgba(255,255,255,0.07)" strokeWidth="1" />
            {Array.from({ length: 11 }).map((_, i) => (
              <line key={'lat'+i} x1={0} x2={1000} y1={(i * 500) / 10} y2={(i * 500) / 10} stroke="rgba(255,255,255,0.05)" strokeDasharray="2 6" />
            ))}
            {Array.from({ length: 21 }).map((_, i) => (
              <line key={'lon'+i} y1={0} y2={500} x1={(i * 1000) / 20} x2={(i * 1000) / 20} stroke="rgba(255,255,255,0.05)" strokeDasharray="2 6" />
            ))}
          ...
          {locations.length === 0 && (
            <div style={{ marginTop: 10, padding: 12, background: c.bg.secondary, border: '1px solid ' + c.border.default, fontSize: 10, color: c.text.muted }}>
              No locations found. (Check <code>src/data/demo.js</code>.)
            </div>
          )}
        </div>
          ...
          {locations.map((loc) => (
            <div key={loc.id} style={{ padding: 12, background: c.bg.card, border: '1px solid ' + c.border.default }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 9, color: loc.status === 'active' ? c.status.active : c.accent.primary }}>{String(loc.status || '').toUpperCase()}</span>
              </div>
