      ...
      {!a ? (
        <div style={{ padding: 24, background: c.bg.card, border: '1px solid ' + c.border.default }}>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button
              onClick={() => { setSection('tools'); setToolsTab('hub'); }}
            ...
            <span style={{ fontSize: 9, color: c.text.muted, alignSelf: 'center' }}>
              (Upload an XER file in the Tools tab to generate real analytics)
            </span>
          </div>
        </div>
      ) : isReal ? (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
            {[
              { k: 'ACTIVITIES', v: a.kpis?.activities || '—', col: c.accent.primary },
              { k: 'MILESTONES', v: a.kpis?.milestones || 0, col: c.chart.design },
              { k: 'MISSING LOGIC', v: a.kpis?.missingLogic || 0, col: (a.kpis?.missingLogic || 0) > 10 ? c.status.warning : c.status.active },
              { k: 'CONSTRAINTS', v: a.kpis?.constraints || 0, col: (a.kpis?.constraints || 0) > 20 ? c.status.warning : c.text.primary },
            ].map((m) => (
              <div key={m.k} style={{ padding: 14, background: c.bg.card, border: '1px solid ' + c.border.default }}>
                ...
                <span style={{ fontSize: 18, color: m.col }}>{m.v}</span>
              </div>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
            {[
              { k: 'CONTRACT START', v: a.kpis?.contractStart || '—' },
              { k: 'CONTRACT FINISH', v: a.kpis?.contractFinish || '—' },
              { k: 'FORECAST FINISH', v: a.kpis?.forecastFinish || '—', col: c.accent.primary },
              { k: 'VARIANCE', v: a.kpis?.varianceDays != null ? `${a.kpis.varianceDays}d` : '—', col: (a.kpis?.varianceDays || 0) >= 0 ? c.status.active : c.status.critical },
            ].map((m) => (
              <div key={m.k} style={{ padding: 14, background: c.bg.card, border: '1px solid ' + c.border.default }}>
                <span style={{ fontSize: 8, color: c.text.muted, display: 'block', marginBottom: 6 }}>{m.k}</span>
                <span style={{ fontSize: 14, color: m.col || c.text.primary }}>{m.v}</span>
              </div>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div style={{ padding: 20, background: c.bg.card, border: '1px solid ' + c.border.default }}>
              <span style={{ fontSize: 9, color: c.text.muted, display: 'block', marginBottom: 10 }}>STATUS BREAKDOWN</span>
              {a.kpis?.statusCounts && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                  {[
                    { k: 'Not Started', v: a.kpis.statusCounts.notStarted, col: c.text.muted },
                    { k: 'In Progress', v: a.kpis.statusCounts.inProgress, col: c.status.active },
                    { k: 'Complete', v: a.kpis.statusCounts.complete, col: c.chart.design },
                  ].map(s => (
                    <div key={s.k} style={{ padding: 12, background: c.bg.secondary, border: '1px solid ' + c.border.default }}>
                      <span style={{ fontSize: 8, color: c.text.muted, display: 'block', marginBottom: 4 }}>{s.k}</span>
                      <span style={{ fontSize: 18, color: s.col }}>{s.v}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ padding: 20, background: c.bg.card, border: '1px solid ' + c.border.default }}>
              <span style={{ fontSize: 9, color: c.text.muted, display: 'block', marginBottom: 10 }}>SCHEDULE HEALTH</span>
              <div style={{ display: 'grid', gap: 8 }}>
                {[
                  { k: 'Open starts', v: a.kpis?.openStart || 0 },
                  { k: 'Open finishes', v: a.kpis?.openFinish || 0 },
                  { k: 'Float median', v: a.kpis?.floatMedian != null ? `${a.kpis.floatMedian}d` : '—' },
                ].map(s => (
                  <div key={s.k} style={{ display: 'flex', justifyContent: 'space-between', padding: 8, background: c.bg.secondary, border: '1px solid ' + c.border.default }}>
                    <span style={{ fontSize: 10, color: c.text.secondary }}>{s.k}</span>
                    <span style={{ fontSize: 10, color: c.text.primary }}>{s.v}</span>
                  </div>
                ))}
              </div>
            </div>
          </div>
          {a.evidence?.drivers?.length > 0 && (
            <div style={{ padding: 20, background: c.bg.card, border: '1px solid ' + c.border.default, marginBottom: 16 }}>
              <span style={{ fontSize: 9, color: c.text.muted, display: 'block', marginBottom: 10 }}>POTENTIAL DRIVERS (low float, near forecast finish)</span>
              <div style={{ display: 'grid', gap: 6 }}>
                {a.evidence.drivers.slice(0, 8).map(d => (
                  <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', padding: 8, background: c.bg.secondary, border: '1px solid ' + c.border.default }}>
                    <span style={{ fontSize: 10, color: c.accent.primary }}>{d.code}</span>
                    <span style={{ fontSize: 10, color: c.text.secondary, flex: 1, marginLeft: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.name}</span>
                    <span style={{ fontSize: 10, color: d.floatDays != null && d.floatDays <= 0 ? c.status.critical : c.text.muted, marginLeft: 10 }}>TF: {d.floatDays != null ? d.floatDays + 'd' : '—'}</span>
                  </div>
                ))}
              </div>
            </div>
          )}
          <button onClick={() => { setSection('tools'); setToolsTab('reader'); }}
            style={{ padding: '10px 14px', background: c.accent.primary, border: 'none', cursor: 'pointer' }}>
            <span style={{ fontSize: 10, color: c.bg.primary }}>OPEN XER READER FOR FULL ACTIVITY VIEW</span>
          </button>
        </>
      ) : (
        <>
        ...
        <div style={{ padding: 16, background: c.bg.card, border: '1px solid ' + c.border.default }}>
          ...
          <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <button onClick={() => fileARef.current?.click()}
              ...
              {baselineFile ? `Baseline: ${baselineFile.name}` : 'Upload Baseline / Prev (XER)'}
            </button>
            <button onClick={() => fileBRef.current?.click()}
              style={{ width: '100%', padding: 10, background: c.bg.secondary, border: '1px solid ' + c.border.default, cursor: 'pointer', color: c.text.primary, fontSize: 10, fontFamily: 'monospace' }}>
              {currentFile ? `Current: ${currentFile.name}` : 'Upload Current (XER)'}
            </button>
          </div>
