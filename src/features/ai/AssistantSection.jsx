...
export default function AssistantSection() {
  ...
  const contextPack = useMemo(() => {
    ...
    return {
      ...
      analysis: analysis ? { kpis: analysis.kpis, highlights: analysis.highlights } : null,
  ...
  const runLocalAnalytics = () => {
    const file = uploadedFile;
    const a = generateAnalytics(file, { demoMode });
    setAnalysis(a);
    const rr = generateRiskRegister(a, { demoMode });
    setRiskRegister(rr);
    const ap = generateActionPlan(a, rr, { demoMode });
    setActionPlan(ap);
    const pb = generatePlaybook(a, rr, ap, { demoMode });
    setPlaybook(pb);
    return { a, rr, ap, pb };
  };

  const runLocalCompare = () => {
    const comp = generateComparison(baselineFile, currentFile, { demoMode });
    setComparison(comp);
    return comp;
  };
  ...
  const send = async () => {
    ...
    try {
      ...
      if (intent === 'analytics' && (uploadedFile || demoMode)) {
        const out = runLocalAnalytics();
        ran = true;
        setMsgs((m) => [...m, { role: 'assistant', ts: nowISO(), content: `✅ Analytics generated.

Key KPIs:
• Forecast finish: ${out.a?.kpis?.forecastFinish || '—'}
...
Next: ask for a director summary or click “Download exec report”.` }]);
      ...
      if (intent === 'compare' && (baselineFile && currentFile)) {
        const comp = runLocalCompare();
        ran = true;
...
Ask: “What changed & why it matters?” for the narrative.` }]);
      ...
      if (intent === 'report') {
        ...
        if (!analysis && (uploadedFile || demoMode)) runLocalAnalytics();
        if (!comparison && (baselineFile && currentFile)) runLocalCompare();
        ran = true;
        downloadExecReport();
