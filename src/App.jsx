...
const systemData = {
  ...
  metrics: { portfolioValue: 3000, totalCapacity: 458, projectsDelivered: 35, onTimeDelivery: 94 },
  
  careerTimeline: [
    { id: 'vantage', company: 'Vantage Data Centers', role: 'Regional Planning Lead – EMEA', period: 'May 2025 - Present', location: 'Dublin, Ireland', type: 'current', logo: '🏢', description: 'Portfolio-level planning across hyperscale data center developments.', highlights: ['Managing IMS for Dublin portfolio (€1.5B+)', 'Overseeing 4 projects: 2 DCs, MFGP, BESS', 'Line managing planners across projects'], value: 1500, projects: 4 },
  ...
  programmes: [
    { 
      ...
      monteCarlo: { 
        distribution: [
          { days: -20, probability: 2 }, { days: -10, probability: 12 }, 
          { days: 0, probability: 25 }, { days: 10, probability: 12 }, { days: 20, probability: 3 }
        ] 
      },
      decisions: [
        { id: 'd1', title: 'IMS RESTRUCTURE', date: '2025-06', context: 'Facilities on independent paths', intervention: 'Integrated Master Schedule', impact: 'Protected RFS', counterfactual: '€4.2M exposure', metrics: { costAvoided: 4200000 } }, 
        { id: 'd2', title: 'CX PARALLELISATION', date: '2025-08', context: 'Sequential Cx overrun', intervention: 'Parallelised systems', impact: 'Absorbed delay', counterfactual: 'CP breach', metrics: { costAvoided: 2800000 } }
      ]
    },
    { 
      ...
      decisions: [
        { id: 'd1', title: 'RESOURCE LEVELLING', date: '2024-09', context: 'Resource conflicts', intervention: 'Cross-project matrix', impact: 'Eliminated conflicts', counterfactual: 'Multiple delays', metrics: { costAvoided: 1800000 } }
      ]
    },
    { 
      ...
      decisions: [
        { id: 'd1', title: 'CP RESEQUENCE', date: '2023-03', context: 'Permit delay', intervention: 'Resequenced works', impact: 'Absorbed delay', counterfactual: '8 week slip', metrics: { costAvoided: 6500000 } }
      ]
    },
    { 
      ...
      projects: [
        ...
        { name: 'Grangecastle DC1', capacity: '20MW', type: 'DC', location: 'Dublin' }, 
        { name: 'Amsterdam DC', capacity: '50MW', type: 'Data Centre', location: 'Netherlands' }, 
        { name: 'Paris DC', capacity: '32MW', type: 'DC', location: 'France' }, 
        { name: 'Warsaw DC', capacity: '48MW', type: 'DC', location: 'Poland' }
      ],
      workstreams: [
        { id: 'design', name: 'DESIGN', progress: 100 }, 
        { id: 'procurement', name: 'PROCUREMENT', progress: 100 }, 
      ...
      decisions: [
        { id: 'd1', title: 'EVM IMPLEMENTATION', date: '2020-06', context: 'No baseline', intervention: 'P6/Excel EVM', impact: 'IBR approved', counterfactual: 'Reactive reporting', metrics: { costAvoided: 1200000 } }
      ]
    }
  ],
  ...
  careerProgression: [
    ...
    { year: '2020', value: 250, complexity: 65 }, 
    { year: '2021', value: 350, complexity: 75 }, 
    { year: '2022', value: 550, complexity: 80 }, 
    { year: '2023', value: 750, complexity: 85 }, 
    { year: '2024', value: 1000, complexity: 90 }, 
  ...
  capabilities: { 
    ...
    governance: { score: 92, tools: ['IMS', 'IBR', 'EOT Analysis'] } 
  }
};

const qsraChecks = [
  { id: '1', name: 'FS Relationships', threshold: '≥70%', category: 'Logic' },
  { id: '2', name: 'Missing Logic', threshold: '≤1', category: 'Logic' },
  { id: '3', name: 'Leads', threshold: '=0%', category: 'Logic' },
  { id: '4', name: 'Lags', threshold: '<15%', category: 'Logic' },
  { id: '5', name: 'Hard Constraints', threshold: '=0', category: 'Constraints' },
  { id: '6', name: 'Soft Constraints', threshold: '<10%', category: 'Constraints' },
  { id: '7', name: 'High Duration', threshold: '<10%', category: 'Duration' },
  { id: '8', name: 'Negative Float', threshold: '=0%', category: 'Float' },
  { id: '9', name: 'High Float', threshold: '<10%', category: 'Float' },
  { id: '10', name: 'Resource Loaded', threshold: '>90%', category: 'Resources' }
];
...
const dcmaChecks = [
  ...
  { id: 4, name: 'Relationship Types', threshold: '< 10%' },
  { id: 5, name: 'Hard Constraints', threshold: '< 5%' },
  { id: 6, name: 'High Float', threshold: '< 5%' },
  { id: 7, name: 'Negative Float', threshold: '= 0%' },
...
function Boot({ onComplete }) {
  ...
  useEffect(() => {
    const timers = [200, 500, 800, 1100, 1400, 1700].map((d, i) => 
      setTimeout(() => setPhase(i + 1), d)
    );
    const complete = setTimeout(onComplete, 2200);
    return () => {
      ...
      <div style={{ width: '100%', maxWidth: 500, padding: 32 }}>
        ...
        {phase >= 6 && (
          <motion.div 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            style={{ padding: 20, border: `1px solid ${c.border.default}`, background: c.bg.secondary, marginTop: 20 }}
          >
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              <div>
                <span style={{ fontSize: 18, color: c.accent.primary }}>€3B+</span>
                <span style={{ fontSize: 8, color: c.text.muted, display: 'block' }}>Portfolio</span>
              </div>
              <div>
                <span style={{ fontSize: 18, color: c.status.active }}>458MW</span>
                <span style={{ fontSize: 8, color: c.text.muted, display: 'block' }}>Capacity</span>
              </div>
              <div>
                <span style={{ fontSize: 18, color: c.chart.design }}>35</span>
                <span style={{ fontSize: 8, color: c.text.muted, display: 'block' }}>Projects</span>
              </div>
            </div>
          </motion.div>
        )}
...
function StatusBar() {
  ...
  const labels = { 
    system: 'SYSTEM', career: 'CAREER', analytics: 'ANALYTICS', 
    programmes: 'PROGRAMMES', decisions: 'DECISIONS', risk: 'RISK', 
    map: 'MAP', capabilities: 'CAPABILITIES', tools: 'TOOLS' 
  };
  ...
  return (
    <div style={{ 
      ...
      padding: '0 16px', fontFamily: 'monospace', zIndex: 100 
    }}>
...
function Nav() {
  ...
  return (
    <nav style={{ 
      width: 180, minHeight: '100vh', paddingTop: 32, 
      background: c.bg.secondary, borderRight: `1px solid ${c.border.default}`, 
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {['executive', 'planner'].map(m => (
            <button 
              ...
              style={{ 
                flex: 1, padding: 6, 
                border: `1px solid ${mode === m ? c.accent.dim : c.border.default}`, 
                background: mode === m ? 'rgba(160,128,64,0.1)' : 'transparent', 
                cursor: 'pointer', fontSize: 8, 
                color: mode === m ? c.accent.primary : c.text.muted 
              }}
            >
              {m.toUpperCase()}
            </button>
          ...
          return (
            <button 
              ...
              style={{ 
                ...
                background: active ? 'linear-gradient(90deg, rgba(160,128,64,0.1), transparent)' : 'transparent', 
      ...
      <div style={{ padding: 20, background: c.bg.card, border: `1px solid ${c.border.default}`, marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button 
            ...
            style={{ padding: '4px 10px', background: 'transparent', border: `1px solid ${c.border.default}`, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}
          >
        ...
        <div style={{ position: 'relative', paddingLeft: 24 }}>
          ...
          {systemData.careerTimeline.map((job, i) => (
            <motion.div 
              ...
              style={{ position: 'relative', marginBottom: 20, paddingLeft: 24 }}
            >
              <div style={{ 
                position: 'absolute', left: -24, top: 4, width: 16, height: 16, 
                background: job.type === 'current' ? c.status.active : c.bg.card, 
                border: `2px solid ${job.type === 'current' ? c.status.active : c.accent.primary}`, 
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
                      <span style={{ fontSize: 9, color: c.text.muted }}>
                        <Calendar size={10} style={{ verticalAlign: 'middle', marginRight: 4 }} />{job.period}
                      </span>
                      <span style={{ fontSize: 9, color: c.text.muted }}>
                        <MapPin size={10} style={{ verticalAlign: 'middle', marginRight: 4 }} />{job.location}
                      </span>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 20 }}>
        {systemData.programmes.map(p => (
          <motion.div 
            ...
            style={{ 
              padding: 16, 
              background: programme?.id === p.id ? 'rgba(160,128,64,0.1)' : c.bg.card, 
              border: `1px solid ${programme?.id === p.id ? c.accent.dim : c.border.default}`, 
              cursor: 'pointer' 
            }}
          >
                ...
                <div key={i} style={{ padding: 12, background: c.bg.secondary, border: `1px solid ${c.border.default}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      <span style={{ fontSize: 9, color: c.text.muted, display: 'block' }}>{proj.type}{proj.location ? ` • ${proj.location}` : ''}</span>
                    </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['analytics', 'decisions', 'risk'].map(s => (
              <button 
                ...
                style={{ padding: '8px 14px', background: 'transparent', border: `1px solid ${c.border.default}`, cursor: 'pointer', fontSize: 9, color: c.text.muted, textTransform: 'uppercase' }}
              >
                {s}
              </button>
            ))}
...
function DecisionsSection() {
  ...
  useEffect(() => {
    if (decision) {
      setPhase(0);
      const timers = [300, 800, 1300, 1800].map((d, i) => setTimeout(() => setPhase(i + 1), d));
          ...
          {p.decisions?.map((d, i) => (
            <div 
              ...
              style={{ 
                ...
                background: decision?.id === d.id ? 'rgba(160,128,64,0.1)' : c.bg.card, 
                border: `1px solid ${decision?.id === d.id ? c.accent.dim : c.border.default}`, 
                cursor: 'pointer' 
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <div style={{ 
                  ...
                  background: decision?.id === d.id ? c.accent.primary : c.border.default, 
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        {['p20', 'p50', 'p80'].map(s => (
          <button 
            ...
            style={{ 
              padding: '8px 12px', 
              border: `1px solid ${scenario === s ? c.accent.dim : c.border.default}`, 
              background: scenario === s ? 'rgba(160,128,64,0.1)' : 'transparent', 
              cursor: 'pointer', fontSize: 9, 
              color: scenario === s ? c.accent.primary : c.text.muted 
            }}
          >
            {s.toUpperCase()}
          </button>
          ...
          {p.risks.map(r => {
            ...
            return (
              <div key={r.id} style={{ padding: 14, background: c.bg.card, border: `1px solid ${c.border.default}`, borderLeft: `3px solid ${col}` }}>
                ...
                <div style={{ height: 4, background: c.border.default }}>
                  <div style={{ width: `${adj}%`, height: '100%', background: col }} />
                </div>
              </div>
            );
          })}
...
function MapSection() {
            ...
            {systemData.locations.map(loc => {
              ...
              return (
                <g key={loc.id} onMouseEnter={() => setHover(loc.id)} onMouseLeave={() => setHover(null)} style={{ cursor: 'pointer' }}>
                  {loc.status === 'active' && (
                    <circle cx={pos.x} cy={pos.y} r="18" fill="none" stroke={col} strokeWidth="1" opacity="0.4">
                  ...
                  {hover === loc.id && (
                    <>
                      <rect x={pos.x + 12} y={pos.y - 30} width={120} height={45} fill={c.bg.elevated} stroke={c.border.default} />
                      <text x={pos.x + 18} y={pos.y - 12} fill={c.text.primary} fontSize="9">{loc.name}</text>
                      <text x={pos.x + 18} y={pos.y} fill={c.text.secondary} fontSize="8">{loc.type}</text>
                      <text x={pos.x + 18} y={pos.y + 12} fill={c.accent.primary} fontSize="8">{loc.capacity}</text>
                    </>
                  )}
                </g>
              );
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 440, overflowY: 'auto' }}>
          {systemData.locations.map(loc => (
            <div 
              ...
              style={{ 
                padding: 12, 
                background: hover === loc.id ? c.bg.elevated : c.bg.card, 
                border: `1px solid ${hover === loc.id ? c.accent.dim : c.border.default}`, 
                cursor: 'pointer' 
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
 ...
 'rgba(239,68,68,0.2)', color: c.status.critical }}>
                      {qsraResults.checks.filter(r => r.status === 'FAIL').length} FAIL
                    </span>
                  </div>
                ...
                <div style={{ padding: 32, textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 11, color: c.text.muted, marginTop: 12 }}>Upload schedule to analyze</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            <div style={{ padding: 20, background: c.bg.card, border: `1px solid ${c.border.default}` }}>
              ...
              <button 
                onClick={runDCMA} 
                disabled={analyzing} 
                style={{ width: '100%', padding: 14, background: c.accent.primary, border: 'none', cursor: 'pointer' }}
              >
                <span style={{ fontSize: 10, color: c.bg.primary }}>
                  {analyzing ? 'ANALYZING... ' + Math.round(progress) + '%' : 'RUN DCMA ANALYSIS'}
                </span>
              </button>
            </div>
            ...
            <div style={{ padding: 20, background: c.bg.card, border: `1px solid ${c.border.default}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                {dcmaResults && (
                  <button 
                    onClick={() => generatePDF('dcma')} 
                    style={{ padding: '6px 12px', background: 'transparent', border: `1px solid ${c.accent.dim}`, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
                  >
                    <Download size={12} color={c.accent.primary} />
                    <span style={{ fontSize: 8, color: c.accent.primary }}>PDF</span>
                  </button>
                )}
              </div>
              {dcmaResults ? (
                <>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 12 }}>
                    <span style={{ fontSize: 56, color: dcmaResults.score >= 85 ? c.status.active : dcmaResults.score >= 70 ? c.status.warning : c.status.critical }}>
                      {dcmaResults.score}
                    </span>
                  ...
                  <div style={{ 
                    padding: 12, 
                    background: dcmaResults.compliance === 'COMPLIANT' ? 'rgba(34,197,94,0.1)' : 'rgba(234,179,8,0.1)', 
                    border: `1px solid ${dcmaResults.compliance === 'COMPLIANT' ? c.status.active : c.status.warning}` 
                  }}>
                    <span style={{ fontSize: 12, color: dcmaResults.compliance === 'COMPLIANT' ? c.status.active : c.status.warning }}>
                      {dcmaResults.compliance}
                    </span>
                  </div>
                </>
              ) : (
...
function AIAssistant() {
  ...
  const [msgs, setMsgs] = useState([{ role: 'assistant', content: "Hi! I'm Aniket's Planning AI. Ask about experience or navigate sections." }]);
  ...
  const respond = (msg) => {
    ...
    if (m.includes('career')) { setSection('career'); return "Navigated to Career."; }
    if (m.includes('tool') || m.includes('qsra')) { setSection('tools'); return "Opening Tools."; }
    if (m.includes('map')) { setSection('map'); return "Showing map."; }
    if (m.includes('contact')) return "📧 latpate.aniket92@gmail.com 📱 +353 894451882";
    if (m.includes('who') || m.includes('aniket')) return "Aniket is Regional Planning Lead at Vantage, managing €1.5B+ portfolio. 8+ years experience.";
    return "Try: 'show career', 'what is QSRA', or 'contact info'.";
  };
    ...
    return (
      <motion.button 
        ...
        style={{ 
          ...
          background: `linear-gradient(135deg, ${c.accent.primary}, ${c.accent.bright})`, 
  ...
  return (
    <motion.div 
      ...
      style={{ 
        ...
        display: 'flex', flexDirection: 'column', zIndex: 1000, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' 
      }}
    >
      <div style={{ padding: '12px 16px', borderBottom: `1px solid ${c.border.default}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setAiOpen(false)} style={{ background: 'transparent', border: 'none', cursor: 'pointer' }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {[0, 1, 2].map(i => (
                  <motion.div 
                    ...
                    style={{ width: 6, height: 6, borderRadius: '50%', background: c.accent.primary }} 
                  />
                ))}
              </div>
            </div>
...
function Main() {
  ...
  const Section = sectionComponents[section] || SystemSection;
